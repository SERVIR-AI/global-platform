...
export const GroundedStrip: FC<{
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5 text-[0.7rem] text-base-content/70">
      <span className="rounded px-1.5 py-0.5 bg-base-200">
        cites {grounded.cited.length} of {citations.length} sources
      </span>
      <CheckChip ok={grounded.phantom_citations.length === 0} label="all citations resolve" />
      ...
      <CheckChip
        ...
        label={
          unverified.length === 0
            ? 'figures cross-checked against evidence'
...
/** Copies the FULL brief with its Sources block, a provenance header (who wrote
 *  it, which checks it passed), and archive links made absolute — the sanctioned
 *  copy path, so what leaves this screen carries working receipts. */
export const CopyBriefButton: FC<{ brief: string; grounded?: Grounded | null; model?: string }> = ({
  brief,
  grounded,
  model,
}) => {
  ...
  const copyText = () => {
    const checks = grounded
      ? `groundedness ${grounded.passed ? 'passed' : 'FAILED'} (attempt ${grounded.attempts ?? 1})` +
        (grounded.numbers_unverified?.length
          ? `; figures not found in evidence: ${grounded.numbers_unverified.join(', ')}`
          : '')
      : 'groundedness report unavailable';
    const header = `> Generated by ${model ?? 'the Global Risk Platform'} — ${checks}.\n\n`;
    // Relative archive links die outside this app; pin them to this origin.
    const absolute = brief.replace(/(archived: )(\/[^\s]+)/g, (_m, p, path) =>
      `${p}${new URL(resolveApiUrl(path), window.location.origin).toString()}`);
