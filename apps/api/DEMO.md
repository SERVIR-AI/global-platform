...
```bash
ask() { curl -s localhost:8001/api/chat -H 'Content-Type: application/json' \
...
Add `"verbose": true` to get a `trace` array narrating route → resolve → boundary →
exposure → overlay. Omit it and `trace` is null.
