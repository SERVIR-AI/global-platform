...
- **☑ S1 — Verification pass.** `conf/raster_schema.yml` + `schema.py` + `rasterstats.windowed_stats` + `verify.py` (+ CLI). All 6 flood-L2 layers verified against the real files; 2 fast + 4 slow tests green. Findings: 3 distinct grids (100 m / 30 m / 0.000833°), `global_pc_h100glob` confirmed millimetres, pop is 0–5 (earlier "0–4" was a 512-px sampling artifact).
- **☑ S1 — Schema coverage.** All 32 reclassed (0–5) catalog tifs now have a contract (9 hazard + 9 risk + 14 vulnerability); a coverage test forbids drift. Raw/continuous layers are declared per-slice (cutoffs/units in S9); forest/mangrove excluded (non-risk). Verify is **fail-closed**: an undeclared layer returns FAIL, so nothing is used unverified. Suite 19 passed / 5 skipped.
