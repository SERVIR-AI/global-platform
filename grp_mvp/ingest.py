...
FLOOD_DRIVE_ID = "1s5ZSmFGLDH3i2u74RRq0MBOyawV3Fh34"  # notes-tab hazard_flood.tif (SEA, severity 1-5)
AREA_CAP_KM2 = 1500.0
BUFFER_DEG = 0.01
...
def flood_source():
    """The SEA-wide flood raster, downloaded once."""
    os.makedirs(os.path.join(CACHE, "_source"), exist_ok=True)
    path = os.path.join(CACHE, "_source", "hazard_flood_sea.tif")
    if not os.path.exists(path):
        if os.path.exists("/tmp/hazard_flood_sea.tif"):
            shutil.copy("/tmp/hazard_flood_sea.tif", path)
        else:
            import gdown
            gdown.download(id=FLOOD_DRIVE_ID, output=path, quiet=True)
    return path
...
def ensure_aoi(place):
    ...
    with rasterio.open(flood_source()) as src:
